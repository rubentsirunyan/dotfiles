# Generated by Powerlevel10k configuration wizard on 2022-03-17 at 13:33 +04.
...
() {
  ...
  function my_git_formatter() {
    ...
    if (( $1 )); then
      ...
      local     staged='%39F'   # blue foreground
      local   modified='%178F'  # yellow foreground
      local  untracked='%251F'   # blue foreground
    ...
    typeset -g gs=${clean}
    (( VCS_STATUS_NUM_STAGED     )) && gs=${staged}
    (( VCS_STATUS_NUM_UNSTAGED   )) && gs=${modified}
    ...
    if [[ -n $VCS_STATUS_LOCAL_BRANCH ]]; then
      ...
      res+="${gs}${(g::)POWERLEVEL9K_VCS_BRANCH_ICON}${branch//\%/%%}"
    fi
    ...
    [[ -z $VCS_STATUS_LOCAL_BRANCH && -z $VCS_STATUS_TAG ]] &&  # <-- this line
      res+="${meta}@${gs}${VCS_STATUS_COMMIT[1,8]}"

    # Show tracking branch name if it differs from local branch.
    if [[ -n ${VCS_STATUS_REMOTE_BRANCH:#$VCS_STATUS_LOCAL_BRANCH} ]]; then
      res+="${meta}:${gs}${(V)VCS_STATUS_REMOTE_BRANCH//\%/%%}"
    fi
    ...
    (( VCS_STATUS_NUM_STAGED     )) && res+=" ${staged}${(g::)POWERLEVEL9K_VCS_STAGED_ICON}${VCS_STATUS_NUM_STAGED}"
    # !42 if have unstaged changes.
    (( VCS_STATUS_NUM_UNSTAGED   )) && res+=" ${modified}${(g::)POWERLEVEL9K_VCS_MODIFIED_ICON}${VCS_STATUS_NUM_UNSTAGED}"
